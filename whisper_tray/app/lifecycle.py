...
class WhisperTrayApp:
    ...
    def _prepare_tray_runtime(self) -> None:
        ...
        from whisper_tray import app as app_package

        self._tray_runtime = self._create_tray_runtime()
        try:
            self._tray_runtime.prepare(self)
        except Exception:
            if not isinstance(self._tray_runtime, app_package.QtTrayRuntime):
                raise
            ...
            self._tray_runtime = app_package.PystrayTrayRuntime()
            self._tray_runtime.prepare(self)
    ...
    def _create_tray_runtime(self) -> TrayRuntime:
        ...
        if tray_backend == "pystray":
            return app_package.PystrayTrayRuntime()

        if app_package.should_use_qt_tray():
            return app_package.QtTrayRuntime()
